...
IMPORTANT: Generate 8-10 comprehensive test cases covering different aspects of the functionality described in the ticket. Ensure each test case is complete and follows the exact format above. Use an appropriate number of steps for each test case based on the complexity of what's being tested."""
    ...
    def enhance_onboarding_tasks(self, context: Dict) -> List[Dict]:
        """
        Enhance onboarding tasks using AI.
        
        Args:
            context: Dictionary containing:
                - employee: Employee information
                - base_tasks: List of base tasks
                - company_docs: Documentation links
                
        Returns:
            List[Dict]: Enhanced tasks with AI-generated improvements
        """
        try:
            # Create prompt for task enhancement
            prompt = self._create_onboarding_prompt(context)
            
            # Get AI suggestions
            response = self._get_completion(prompt)
            
            if not response:
                logger.error("Failed to get AI response for onboarding tasks")
                return context['base_tasks']
            
            # Parse and validate AI suggestions
            enhanced_tasks = self._parse_enhanced_tasks(response)
            
            # Merge AI suggestions with base tasks
            return self._merge_task_suggestions(context['base_tasks'], enhanced_tasks)
            
        except Exception as e:
            logger.error(f"Error enhancing onboarding tasks: {str(e)}")
            return context['base_tasks']
    
    def _create_onboarding_prompt(self, context: Dict) -> str:
        """Create prompt for onboarding task enhancement."""
        employee = context['employee']
        base_tasks = context['base_tasks']
        
        return f"""As an expert in employee onboarding, enhance and personalize these onboarding tasks for a new team member:
...
Please suggest additional tasks or modifications to existing tasks that would:
1. Make the onboarding more personalized to the role and team
2. Include specific technical skills or tools relevant to the role
3. Add learning objectives and success criteria
4. Suggest relevant documentation or resources
5. Add mentorship or buddy system components

Format each task as a JSON object with:
- title: Task title
- description: Detailed description with bullet points
- category: Task category (Setup, Technical, Process, etc.)
- estimated_days: Estimated days to complete
- dependencies: List of prerequisite task titles (optional)
- resources: List of relevant documentation links (optional)

Focus on making the tasks practical, actionable, and aligned with the role requirements."""